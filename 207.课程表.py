...
#             return True
#         # 邻接表
#         adj = [set() for _ in range(numCourses)] #list[list[]]
#         # print(adj)
#         # 建立入度表
#         # 注意：邻接表存放的是后继 successor 结点的集合
#         in_degrees = numCourses*[0]
#         for i, j in prerequisites:
#             in_degrees[i] += 1  # 需要提前完成的结点的入度加1
#             adj[j].add(i)       # 记录的是 j的出度i  
...
#         while queue:
#             top = queue.pop(0)
#             counter += 1  # 每次弹出一个出度为0的结点 就应该把counter+1  
#             for successor in adj[top]:  
#                 in_degrees[successor] -= 1  # 这个弹出结点的所有指向的结点的出度都应该减一
#                 if in_degrees[successor] == 0:  # 把结点更新完是0的结点再次添加到队列中
#                     queue.append(successor)
        
#         return counter == numCourses # 如果说修完的课程和给的相同那么能修完
